...
def pick_col_fuzzy(cols, candidates, min_score=70):
    ...
    expanded = list(
        dict.fromkeys(
            candidates
            + [c.replace("ç", "c").replace("ã", "a").replace("õ", "o") for c in candidates]
        )
    )
...
if not COL_DESC:
    raise ValueError(
        "Não identifiquei a coluna de descrição/sintoma. "
        "Veja as colunas listadas e, se necessário, ajuste o código."
    )
...
def add_tree_rows(tid, desc, verifs, sol, sol2, causa, keywords):
    rows.append(
        {
            ...
            "OPT1_LABEL": "Sim",
            "OPT1_NEXT": "chk_1" if verifs else "leaf_final",
            "OPT2_LABEL": "Não",
            "OPT2_NEXT": "leaf_alt",
            "OPT3_LABEL": "",
            "OPT3_NEXT": "",
    ...
    finale = []
    if causa:
        finale.append(f"Causa: {causa}")
    if sol:
        finale.append(f"Solução: {sol}")
    if sol2:
        finale.append(f"Complemento: {sol2}")
    if not finale:
        finale.append("Solução: -")
...
def ord_key(nid):
    ...
    if str(nid).startswith("chk_"):
        try:
            return 1 + int(str(nid).split("_", 1)[1])
        except Exception:
            return 50
    if nid == "leaf_final":
        return 100
