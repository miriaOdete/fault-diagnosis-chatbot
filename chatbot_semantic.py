...
from rapidfuzz import process, fuzz  # <— híbrido com fuzzy
...
class SemanticRouter:
    ...
    def _load_q34(self):
        if not os.path.exists(self.excel_q34):
            self.df_q = None
            return
        df = pd.read_excel(self.excel_q34, sheet_name=self.aba)
    ...
    def pick_tree(self, user_problem: str, limiar=0.45):  # ↓ antes 0.50
        ...
        # log opcional:
        print(f"[ROUTER] best_tree={self.tree_map[best_idx]} final={best:.2f} cos={sims[best_idx]:.2f} fuzzy={fuzzys[best_idx]:.2f}")
    ...
    def fallback_q34(self, user_problem: str):
        ...
        LIMIAR = 0.45  # ↓ antes 0.55 / 0.60
        if best_idx < 0 or best_val < LIMIAR:
            # Mostra sugestões em vez de dizer que não encontrou
            sugest = []
            for i in idxs:
                sugest.append(f"- {texts[i]}")
