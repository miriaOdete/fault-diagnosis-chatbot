...
PATTERNS = [
    ...
    (r"\b(alimentacao|alimentação|energia|tensao|tensão)([\w\s]*)",
     lambda *_g: "Há problema de alimentação/energia (tensão fora do padrão, instabilidade)?"),
    (r"\b(lubrificacao|lubrificação) (insuficiente|inadequada|ausente)",
     lambda *_g: "A lubrificação está ausente ou insuficiente no ponto indicado?"),
]
...
def cause_to_question(cause_text: str) -> str:
    ...
    if base.lower().startswith(("o ","a ","os ","as ")):
        ...
        q = (("Você observou " + base[0].lower() + base[1:]) + "?").strip()
...
class FailureFlowEngine:
    ...
    def _build_domain_vocab_selective(self, min_freq: int = 3):
        """Apenas ALL‑CAPS, tokens com dígitos/hífens ou palavras com frequência >= min_freq entram na whitelist."""
        freq: Dict[str,int] = {}
            ...
            try:
                with open(self.custom_vocab_path, "r", encoding="utf-8") as f:
                    for line in f:
                        w = line.strip()
                        if w:
                            self.user_vocab.add(w); self.user_vocab.add(w.lower())
            except Exception:
                pass
    ...
    def _fit_from_excel(self):
        ...
        C_ACTION = ["Ação Preventiva Recomendada/Troubleshooting","Ação Preventiva","Troubleshooting","Ações Recomendadas","Ação Recomendada","Acoes Preventivas"]
        ...
        self.vectorizer = TfidfVectorizer(min_df=1, ngram_range=(1,2), strip_accents="unicode")
    ...
    # --------------- API principal ---------------
    def pick_mode(self, user_text: str, limiar_ok: float = 0.45) -> Tuple[Optional[str], float, List[str]]:
    ...
    def _actions_for_causes(self, mode: str, effect: str, chosen_causes: List[str]) -> List[str]:
        ...
        for _, row in sel.iterrows():
            ...
            if not act_txt:
                continue
            if not cause_txt:
                actions.append(act_txt); continue
    ...
    def _should_skip_token(self, w: str) -> bool:
        ...
        if any(ch.isdigit() for ch in w): return True
        if w.isupper(): return True
        if "-" in w or "/" in w or "_" in w: return True
        return False

    def _plausible(self, original: str, suggestion: str) -> bool:
        """Aceita apenas correções próximas (distância de edição <= 2)."""
        if not suggestion: return False
        if original.lower() == suggestion.lower(): return False
    ...
    def _correct_sentence(self, text: str) -> str:
        ...
        parts = re.findall(r"\w+|[^\w\s]", str(text), flags=re.UNICODE)
        out = []

        for p in parts:
            if re.fullmatch(r"[^\w\s]", p, flags=re.UNICODE):
                out.append(p); continue

            w = p

            # Whitelist manual/domínio
        ...
        for i, t in enumerate(out):
            if i == 0:
                ...
                if re.fullmatch(r"[^\w\s]", t, flags=re.UNICODE):
                    result += t
                else:
    ...
    def build_questions(self, mode: str, effect: str, max_q: int = 5) -> Dict[str, Any]:
        ...
        if not limited:
            first = causes[0]
            limited = [{"text": cause_to_question(first), "covers": [0]}]

        # Correção ortográfica nas PERGUNTAS
        for q in limited:
    ...
    def finalize(self, diag_state: Dict[str, Any], top_k: int = 3) -> str:
        ...
        # Correção ortográfica na resposta final
        ...
        linhas = ["✅ Diagnóstico concluído", ""]
        linhas += ["🔹 Causa(s) provável(is):"]
